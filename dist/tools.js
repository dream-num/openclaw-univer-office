...
import { resolveWorkspaceInputPath, resolveWorkspaceOutputPath, } from "./cli.js";
...
function requiredStrings(params, key) {
    const value = params[key];
...
function jsonToolResult(command, output, guidance) {
    const details = {
        ok: true,
        command,
        result: output.data,
...
function textToolResult(command, output) {
...
function createConnectTool({ context, runner }) {
    return {
        name: "univer_office_connect",
        label: "Univer Office Connect",
        description: "Check Univer Workspace sign-in, start browser approval, complete it only after user confirmation, or list accessible Spaces.",
...
function createFilesTool({ context, runner }) {
    return {
        name: "univer_office_files",
        label: "Univer Office Files",
        description: "Discover personal and team Office files by stable Space, Node, Resource, and Unit identity before opening a task Worktree.",
        parameters: Type.Object({
            ...
            resource_kind: Type.Optional(Type.Enum(["none", "univer", "blob"], { type: "string" })),
            unit_type: Type.Optional(Type.Enum(["sheet", "doc", "slide", "base", "board"], { type: "string" })),
        }, { additionalProperties: false }),
        async execute(_toolCallId, params, signal) {
            const action = requiredString(params, "action");
            const cwd = resolveWorkspaceDir(context);
...
function requireSpaceForScope(params, scope) {
...
async function buildWorktreeArgs(params, context, config) {
    const action = requiredString(params, "action");
    ...
    if (action === "import") {
        const sourcePath = await resolveWorkspaceInputPath(resolveWorkspaceDir(context), requiredString(params, "source_path"));
        const args = [
            "import",
            "--file",
            sourcePath,
...
function createWorktreeTool(dependencies) {
    const { context, runner } = dependencies;
    return {
        name: "univer_office_worktree",
        label: "Univer Office Worktree",
        description: "Create an isolated Office task, control Team Space visibility, stage or create a Unit, submit it for review, reopen it, or perform user-approved merge/discard.",
        parameters: Type.Object({
            action: Type.Enum([
                "list",
                "get",
...
function createContentTool({ context, runner }) {
    return {
        name: "univer_office_content",
        label: "Univer Office Content",
        description: "Load version-matched guidance, inspect exact Facade APIs, edit one staged Unit, and verify stored or rendered content.",
        parameters: Type.Object({
            action: Type.Enum(["guidance", "api_find", "api_show", "execute", "inspect", "lint"], { type: "string" }),
            skill: Type.Optional(Type.Enum(["core", "sheet", "doc", "slide", "base", "board", "embed", "cross-unit-formula"], { type: "string" })),
            ...
            target: Type.Optional(Type.Enum(["workbook", "worksheet", "range", "presentation", "slide", "document", "paragraph"], { type: "string" })),
...
async function loadScreenshotImages(workspaceDir, data, maximum) {
...
function createHandoffTool({ config, context, runner }) {
    return {
        name: "univer_office_handoff",
        label: "Univer Office Handoff",
        description: "Render model-visible PNG previews or export an Office file inside the active OpenClaw workspace.",
        parameters: Type.Object({
        ...
        async execute(_toolCallId, params, signal) {
            ...
            const outputPath = await resolveWorkspaceOutputPath(workspaceDir, requiredString(params, "output_path"));
            ...
            if (action === "export") {
                ...
                const details = {
                    ...
                    path: outputPath,
                    result: output.data,
                    guidance: "Use OpenClaw's normal message/file delivery surface if the user asked to receive this export.",
                };
                return {
                    content: [{ type: "text", text: JSON.stringify(details, null, 2) }],
                    details,
            ...
            if (action === "screenshot") {
                ...
                const images = await loadScreenshotImages(workspaceDir, output.data, config.screenshotMaxImages);
                ...
                const details = {
                    ...
                    paths,
                    result: output.data,
                    guidance: "The PNGs are visible to the model. Use OpenClaw's normal message/file delivery surface if the user asked to receive them.",
                };
                return {
                    content: [
...
export function createUniverOfficeTools(dependencies) {
    return [
