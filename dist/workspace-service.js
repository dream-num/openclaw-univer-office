...
function workspaceOrigin(config) {
...
async function waitForReady(child, origin) {
    const deadline = Date.now() + STARTUP_TIMEOUT_MS;
    while (Date.now() < deadline) {
        if (child.exitCode !== null || child.signalCode !== null) {
            throw new Error(`Self-hosted Univer Workspace exited before startup (code=${child.exitCode ?? "none"}, signal=${child.signalCode ?? "none"})`);
        }
        try {
...
function forwardLogs(child, context) {
    child.stdout?.setEncoding("utf8");
    child.stderr?.setEncoding("utf8");
    child.stdout?.on("data", (chunk) => {
        const message = chunk.trim();
        if (message)
            context.logger.info(`[univer-workspace] ${message}`);
    });
    child.stderr?.on("data", (chunk) => {
        const message = chunk.trim();
        if (message)
            context.logger.warn(`[univer-workspace] ${message}`);
    });
}
async function waitForSpawnReady(child, origin) {
    let rejectSpawn;
    const spawnFailure = new Promise((_resolve, reject) => {
        rejectSpawn = reject;
    });
    const onError = (error) => rejectSpawn?.(error);
...
export function createSelfHostedWorkspaceService(config, cliPath) {
    let child;
    return {
        id: "univer-workspace",
        async start(context) {
            ...
            try {
                ...
                const configured = await runCommandWithTimeout([cliPath, "config", "set", "workspace.origin", origin], {
                    timeoutMs: 30_000,
                    killProcessTree: true,
                    maxOutputBytes: 64 * 1024,
                });
                if (configured.termination !== "exit" || configured.code !== 0) {
                    throw new Error(`Could not configure Univer Workspace CLI for ${origin}: ${configured.stderr.trim() || configured.termination}`);
                }
            }
            catch (error) {
